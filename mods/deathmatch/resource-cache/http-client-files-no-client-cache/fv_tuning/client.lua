...
function render()
    ...
    dxDrawText("Cash: "..e:getServerColor("servercolor",true)..formatMoney(getElementData(localPlayer,"char >> money"))..white.." dt\nPremium Point: "..e:getServerColor("blue",true)..formatMoney(getElementData(localPlayer,"char >> premiumPoints"))..white.." PP",20*rel,sy-192*rel,sx,sy-192*rel+192*rel,tocolor(255,255,255),1,e:getFont("rage",15*rel),"left","center",false,false,false,true);
    ...
    elseif menuType == "submenu" then 
        ...
        shadowedText("To go back press "..red2.."BACKSPACE"..white..".",10,sy/2+h/2+5,310,h,tocolor(255,255,255),1,font,"center","top",false,false,false,true);
    elseif menuType == "type" then 
        if sMenu == 1 then --Teljesítmény
            ...
            for k,v in pairs(menu[sMenu]["submenu"][sSubmenu][2]) do 
                ...
                if currentTuning and currentTuning[sSubmenu] and currentTuning[sSubmenu] == k then 
                    ...
                    if v[3] then 
                        ...
                        if cost == 0 then 
                            cost = e:getServerColor("blue",true).."Free"
                        else 
                            cost = colorMoney(cost,false)..white.."dt";
                        end
                    end
            ...
            shadowedText("Buying: "..sColor2.."Enter"..red2.."BACKSPACE"..white.."to go back.",10,sy/2+h/2+5,310,h,tocolor(255,255,255),1,font,"center","top",false,false,false,true);
        end
        if sMenu == 2 then --Optika
            ...
            for k,v in pairs(cache[1]) do 
                ...
                if k == 1 then 
                    shadowedText("Nothing",25,sy/2-70-h/2-30+(k*40),310,sy/2-70-h/2-30+(k*40)+30,tocolor(255,255,255),1,font,"left","center");
                else 
        ...
        if sMenu == 3 then --Szinezés 
            if sSubmenu < 4 then --Szinezések 
                ...
                local cost = colorMoney(menu[sMenu]["submenu"][sSubmenu][2][2],false)..white.."dt";
                shadowedText(cost,25,sy/2-42/2+6,295,sy/2-42/2+6+30,tocolor(255,255,255),1,font,"right","center",false,false,false,true);

                shadowedText("Buying: "..sColor2.."Enter"..red2.."BACKSPACE"..white.."to go back.",10,sy/2+42/2+5,310,42,tocolor(255,255,255),1,font,"center","top",false,false,false,true);
            end
            if menu[sMenu]["submenu"][sSubmenu][2] == 4 then --PaintJob 
                ...
                for k,v in pairs(cache) do 
                    ...
                    if v[1] == "none" then 
                        shadowedText("Nincs",25,sy/2-h/2-30+(k*40),310,sy/2-h/2-30+(k*40)+30,tocolor(255,255,255),1,font,"left","center");
                    else 
                    ...
                    if tostring(getElementData(hitVeh,"tuning.paintJob") or "none") == tostring(v[1]) then 
                        shadowedText("",20,sy/2-h/2-30+(k*40),295,sy/2-h/2-30+(k*40)+30,tocolor(orange[1],orange[2],orange[3]),1,icons,"right","center",false,false,false,true);
                        shadowedText("Jelenlegi",20,sy/2-h/2-30+(k*40),275,sy/2-h/2-30+(k*40)+30,tocolor(orange[1],orange[2],orange[3]),1,font,"right","center",false,false,false,true);
                    else 
        ...
        if sMenu == 4 then --Extrák
            ...
            if sSubmenu == 4 then --Rendszám
                ...
                dxDrawText("Max 8 karakter!",sx-200,sy/2+20,sx-200+200,0,tocolor(255,255,255),1,e:getFont("rage",12),"center","top");
                --------------
        ...
        if button == "enter" then 
            ...
            if sMenu == 4 then --Extrák
                if sSubmenu == 4 then --Rendszám
                    ...
                    exports.fv_dx:dxCreateEdit("tuning.plate",(getElementData(hitVeh,"veh:rendszam") or getVehiclePlateText(hitVeh)),"License plate number",sx-195,sy/2-15,190,30);
                end
                if sSubmenu == 8 then --Variáns
            ...
            if sMenu == 4 then --Extrák
                ...
                if sSubmenu == 4 then --Rendszám
                    ...
                    if text == "" or text == " " or string.len(text) < 2 then 
                        exports.fv_infobox:addNotification("warning","The license plate you entered is incorrect!");
                        return;
                    end
        ...
        if sMenu == 1 or (sMenu == 4 and sSubmenu ~= 4) then --Teljesítmény + kerék szélesség + neon + meghajtás + LSD ajtó + Fordulószög
            if button == "arrow_d" then 
                if changeTick+500 > getTickCount() then 
                    exports.fv_infobox:addNotification("warning","Lassabban!");
                    return;
                end
