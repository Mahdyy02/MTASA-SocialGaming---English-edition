...
local brickWallShader
local effectParts

local grassShader
local roadShader
local road2Shader
local paveDirtyShader
local paveStretchShader
local barkShader
local rockShader
local mudShader
local concreteShader
local sandShader

local detail22Texture
local detail58Texture
local detail68Texture
local detail63Texture
local dirtyTexture
local detail04Texture
local detail29Texture
local detail55Texture
local detail35TTexture
----------------------------------------------------------------
-- enableDetail
----------------------------------------------------------------
function enableDetail()
	...
	-- Load textures
	detail22Texture = dxCreateTexture('shaders/hd/detail22.png', "dxt3")
	detail58Texture = dxCreateTexture('shaders/hd/detail58.png', "dxt3")
	detail68Texture = dxCreateTexture('shaders/hd/detail68.png', "dxt1")
	detail63Texture = dxCreateTexture('shaders/hd/detail63.png', "dxt3")
	dirtyTexture = dxCreateTexture('shaders/hd/dirty.png', "dxt3")
	detail04Texture = dxCreateTexture('shaders/hd/detail04.png', "dxt3")
	detail29Texture = dxCreateTexture('shaders/hd/detail29.png', "dxt3")
	detail55Texture = dxCreateTexture('shaders/hd/detail55.png', "dxt3")
	detail35TTexture = dxCreateTexture('shaders/hd/detail35T.png', "dxt3")

	-- Create shaders
	local brickWallShader, tec = getBrickWallShader()
	...
	for _,part in ipairs(effectParts) do
		btAllValid = part and btAllValid
	end

	bEffectEnabled = true

	if not btAllValid then
...
function getMakeShader(v)
	...
	local shader,tec = dxCreateShader ( "shaders/hd/detail.fx", 1, 100 )
	if shader then
