...
function startShaderResource()
 ...
 skydome_shader = dxCreateShader ( "shaders/sky/shaders/shader_skydome.fx",0,0,true,"object" )
 null_shader = dxCreateShader ( "shaders/sky/shaders/shader_null.fx" )
 skydome_texture=dxCreateTexture("shaders/sky/textures/skydome.jpg")
 if not skydome_shader or not null_shader or not skydome_texture then 
  outputChatBox('Nem lehet létrehozni az ég textúrát!',255,0,0)
  return
 end
 ...
 txd_skybox = engineLoadTXD('shaders/sky/models/skybox_model.txd')
 engineImportTXD(txd_skybox, modelID)
 dff_skybox = engineLoadDFF('shaders/sky/models/skybox_model.dff', modelID)
