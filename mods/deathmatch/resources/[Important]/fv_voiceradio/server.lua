            ...
            for k,v in pairs(getElementsByType("player")) do 
                if getElementData(v,"loggedIn") then 
                    if getElementData(player,"char >> radiof") == getElementData(v,"char >> radiof") then 
                        outputChatBox("#00D0FF" .. getElementData(player,"char >> name") .. " says (radio): ".. msg, v, 255, 255, 255, true);
                ...
                if not isPedDead(v) and v ~= player then 
                    ...
                    if distance < 10 then 
                        if getElementDimension(player) == getElementDimension(v) and getElementInterior(player) == getElementInterior(v) then 
                            outputChatBox(getElementData(player,"char >> name") .. " says (radio): ".. msg, v, 255, 255, 255, true);
                        end
                    end
                end
            end
            ...
            for k, v in ipairs(getElementsByType("player")) do
                if getElementData(v, "loggedIn")  then
                    ...
                    if x and not (id == 68 or id == 56) then
                        outputChatBox("#F62459[" .. faction[1] .. "] " .. getElementData(player,"char >> name") .. " says (radio): " .. msg, v, 255, 100, 0, true)
                        triggerClientEvent(v, "radio.sound", v)
                    end
                end
            end
...
function govCommand(player,command,...)
    if getElementData(player,"loggedIn") then 
        ...
        if faction then 
            ...
            for k,v in pairs(getElementsByType("player")) do 
                if getElementData(v,"loggedIn") then 
                    outputChatBox(color.."======["..faction[2].." - Call]======",v,255,255,255,true);
