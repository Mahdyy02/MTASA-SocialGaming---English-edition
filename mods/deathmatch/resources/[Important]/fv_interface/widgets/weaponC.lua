...
addEventHandler("onClientResourceStart", getResourceRootElement(), function()
	for i=0, 46 do 
		if fileExists("widgets/weapons/"..i..".png") then 
			weaponTextures[i] = dxCreateTexture("widgets/weapons/"..i..".png","dxt3");
		end
	end
...
addEventHandler("onClientPlayerWeaponFire", getLocalPlayer(),
	function(weaponID)
		...
		if overheatIncreaseValues[weaponID] then
			...
			overheatValue[currentWeaponDatabaseID] = overheatValue[currentWeaponDatabaseID] + overheatIncreaseValues[weaponID]
...
addEventHandler("hud.weaponOverheatSound", getRootElement(), function ()
	attachElements(playSound3D("widgets/overheat.mp3", getElementPosition(source)), source);
end);

addEventHandler("onClientRender",root,function()
	if not getElementData(localPlayer,"loggedIn") or not getElementData(localPlayer,"togHUD") then return end;
	if isWidgetShowing("weapon") then
        local x,y,w,h = getWidgetDatas("weapon");
		local playerWeapon = getPedWeapon(localPlayer);
		dxDrawImage(x,y,256,128,weaponTextures[playerWeapon],0,0,0,tocolor(255,255,255),true);
