...
float4 processShader(float2 TextureCoordinate : TEXCOORD0) : COLOR0
{
	...
	for(int i = 0; i < 16; i++)
    {
        float2 coord= TextureCoordinate.xy + (poisson[i] / UVSize * 4.5);
...
technique initShader
{
    pass Pass1
    {
        PixelShader = compile ps_2_0 processShader();
    }
}