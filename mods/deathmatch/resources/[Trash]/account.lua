...
addEventHandler("acc.register",root,function(player,name,pw,mail)
    dbQuery(function(qh)
        ...
        if #result > 0 then 
            exports.fv_infobox:addNotification(player,"error","Felhasználónév foglalt!");
            return;
        else
            qh = false;
            result = {};
            dbQuery(function(qh)
                ...
                if #result > 0 then 
                    exports.fv_infobox:addNotification(player,"error","Ehhez a számítógéphez már van account társítva!");
                    return;
                else 
                    qh = false;
                    result = {};
                    ...
                    if sql then 
                        exports.fv_infobox:addNotification(player,"success","Sikeres regisztráció, jelentkezz be!");
        ...
        if #result > 0 then 
            for k,v in pairs(result) do 
                if v.serial ~= getPlayerSerial(player) and serial ~= "0" then           
                    exports.fv_infobox:addNotification(player,"error","Account nem a te gépedhez van társítva!");
                    return;
                end
                if v.online == 1 then 
                    exports.fv_infobox:addNotification(player,"error","Ez az account már használatban van!");
                    return;
                end
                dbExec(sql, "UPDATE accounts SET banDatas='[ false ]', banEnd='0000-00-00' WHERE banEnd<NOW()");
                ...
                if banEnd and banDatas then 
                    kickPlayer(player,"Bannolva lettél!","Admin: "..banDatas[1].." | Napok: "..banDatas[3].." | Lejár: "..banEnd.." \nIndok: "..banDatas[2]);
                    return;
                end
                ...
                exports.fv_infobox:addNotification(player,"success","Sikeres bejelentkezés!");
            end
        else 
            exports.fv_infobox:addNotification(player,"error","Hibás felhasználónév vagy jelszó!");
            return;
        end
...
function loadCharacter(player,id)
    dbQuery(function(qh)
        ...
        if #result > 0 then 
            ...
            dbQuery(function(qh)
                ...
                if #result > 0 then 
                    for k,v in pairs(result) do 
                        ...
                        exports.fv_logs:createLog("MTALOGIN",getElementData(player,"char >> name").." bejelentkezett.",player);
            ...
            exports.fv_infobox:addNotification(player,"info","Még nincs karaktered, hozz létre egy újat.");
...
addEventHandler("acc.charCreate",root,function(player,name,height,weight,age,skin,gender)
    ...
    if sql then 
        exports.fv_infobox:addNotification(player,"success","Sikeres karakter létrehozás!");
        loadCharacter(player,accid);
    else 
        exports.fv_infobox:addNotification(player,"error","Sikertelen karakter létrehozás! (MySQL hiba)");
        return;
    end
end);
...
function savePlayer(source)
    if getElementData(source, "loggedIn") then
        ...
        if getElementData(source,"admin >> duty") then 
            skin = (getElementData(source,"skin.save") or 1);
        end
        ...
        local coins = getElementData(source,"kari->Coin");
